...
//        val fontRoute = File(nextLine())
        val fontRoute = File("fonts/Kawashiro_Pump/chifont.ttf")

        print("가사 경로: ")
//        val lyricsRoute = File(nextLine())
        val lyricsRoute = File("fonts/Kawashiro_Pump/mappers.txt")

        print("색(R,G,B,A): ")
//        val RGBA = nextLine().split(",").map { it.toInt() };
        val RGBA = listOf(255, 255, 255, 230)

        print("색(R,G,B,A): ")
//        val RGBABackground = nextLine().split(",").map { it.toInt() };
        val RGBABackground = listOf(255, 113, 181, 180)
