...
//        val fontRoute = File(nextLine())
        val fontRoute = File("fonts/Paddling Blue/Chihaya Jun.ttf")

        print("가사 경로: ")
//        val lyricsRoute = File(nextLine())
        val lyricsRoute = File("fonts/Paddling Blue/paddling blue lyrics.txt")

        print("색(R,G,B,A): ")
//        val RGBA = nextLine().split(",").map { it.toInt() };
        val RGBA = listOf(255, 255, 255, 255)

        print("색(R,G,B,A): ")
//        val RGBABackground = nextLine().split(",").map { it.toInt() };
        val RGBABackground = listOf(255, 113, 181, 0)
